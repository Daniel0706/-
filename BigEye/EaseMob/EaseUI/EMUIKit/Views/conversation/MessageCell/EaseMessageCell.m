...
@property (nonatomic) NSLayoutConstraint *messageStatusWidthConstraint;
                         ...
                         model:(id<IMessageModel>)model
{
    ...
    _messageStatus = [[UIButton alloc] init];
    _messageStatus.translatesAutoresizingMaskIntoConstraints = NO;
    _messageStatus.imageView.contentMode = UIViewContentModeScaleAspectFit;
    [_messageStatus setImage:[UIImage imageNamed:@"EaseUIResource.bundle/messageSendFail"] forState:UIControlStateNormal];
//    [_messageStatus addTarget:self action:@selector(statusAction) forControlEvents:UIControlEventTouchUpInside];
    _messageStatus.hidden = YES;

    [self.contentView addSubview:_messageStatus];
...
//    [_statusButton setImage:[UIImage imageNamed:@"EaseUIResource.bundle/messageSendFail"] forState:UIControlStateNormal];
    ...
    _statusButton.backgroundColor = RGB(204, 204, 204);
    [_statusButton setTitle:@"同步到在线任务" forState:UIControlStateNormal];
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        switch (messageType) {
            ...
            case EMMessageBodyTypeFile:
            {
                ...
                _bubbleView.videoTagView.image = [UIImage imageNamed:@""];
...
{
    //bubble view
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.bubbleView attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeBottom multiplier:1.0 constant:-35]];
    ...
    // message statusButton
    
    self.messageStatusWidthConstraint = [NSLayoutConstraint constraintWithItem:self.messageStatus attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:self.messageStatusSize];
    [self addConstraint:self.messageStatusWidthConstraint];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.messageStatus attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:self.messageStatus attribute:NSLayoutAttributeWidth multiplier:1.0 constant:0]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.messageStatus attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:-10]];
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.statusButton attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:20]];
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.statusButton attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeCenterX multiplier:1.0 constant:0]];
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.activity attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:-10]];
    //    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.fileStatsLabel attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.fileStatsLabel attribute:NSLayoutAttributeRight multiplier:1.0 constant:EaseMessageCellPadding]];
...
- (void)_updateMessageStatusButtonWidthConstraint
{
    if (_messageStatus) {
        [self removeConstraint:self.messageStatusWidthConstraint];
        
        self.messageStatusWidthConstraint = [NSLayoutConstraint constraintWithItem:self.messageStatus attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:0 constant:self.messageStatusSize];
        [self addConstraint:self.messageStatusWidthConstraint];
    }
}
...
//                _bubbleView.textLabel.attributedText = [[EaseEmotionEscape sharedInstance] attStringFromTextForChatting:model.text textFont:self.messageTextFont];
            ...
            case EMMessageBodyTypeFile:
            {
                ...
                NSArray *fileArray = [_model.fileName componentsSeparatedByString:@"."];
                NSString *fileExtension = fileArray.lastObject;
                _bubbleView.fileIconView.image = [UIImage chooseFileImageWithFileName:fileExtension];
...
- (void)setMessageStatusSize:(CGFloat)messageStatusSize{
    _messageStatusSize = messageStatusSize;
    [self _updateMessageStatusButtonWidthConstraint];
}
