...
@property (strong, nonatomic) UIButton *feedButton;//意见反馈
@property (strong, nonatomic) UIButton *adviceButton;//商务咨询
...
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:150 type:EMChatToolbarTypeGroup feedBack:0];
    if (self) {
...
- (instancetype)initWithFrame:(CGRect)frame
                     ...
                     feedBack:(NSInteger)feedBack
{
    self = [self initWithFrame:frame horizontalPadding:8 verticalPadding:5 inputViewMinHeight:36 inputViewMaxHeight:150 type:type feedBack:feedBack];
    if (self) {
...
- (void)_setupSubviews:(NSInteger)feedBack
{
    //backgroundImageView
    _backgroundImageView = [[UIImageView alloc] initWithFrame:self.bounds];
    _backgroundImageView.autoresizingMask = UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
    _backgroundImageView.backgroundColor = RGB(255, 255, 255);
    _backgroundImageView.layer.borderWidth = 1;
    _backgroundImageView.layer.borderColor = [RGB(223, 223, 225)CGColor];
    ...
    //商务咨询
    self.adviceButton = [[UIButton alloc] initWithFrame:CGRectMake(50, 1, (ScreenWidth-50)/2, _backgroundImageView.frame.size.height - 2)];
    [self.adviceButton setTitle:@"商务咨询" forState:UIControlStateNormal];
    self.adviceButton.titleLabel.font = [UIFont systemFontOfSize:14];
    self.adviceButton.hidden = YES;
    
    [self.adviceButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    self.adviceButton.backgroundColor = RGB(254, 255, 255);
    [self.adviceButton addTarget:self action:@selector(adviceAction:) forControlEvents:UIControlEventTouchUpInside];
    self.adviceButton.layer.borderWidth = 0.5;
    self.adviceButton.layer.borderColor = [RGB(199, 200, 201) CGColor];
    [self addSubview:self.adviceButton];
    UIImageView *bottomImage = [[UIImageView alloc]initWithFrame:CGRectMake((ScreenWidth-50)/2-14, _backgroundImageView.frame.size.height-14, 10, 10)];
    bottomImage.image = [UIImage imageNamed:@"商务咨询弹窗提醒-小三角"];
    [self.adviceButton addSubview:bottomImage];
    
    
    //意见反馈
    self.feedButton = [[UIButton alloc] initWithFrame:CGRectMake((ScreenWidth-50)/2+50, 1, (ScreenWidth-50)/2, _backgroundImageView.frame.size.height - 2)];
    self.feedButton.backgroundColor = RGB(254, 255, 255);
    self.feedButton.hidden = YES;
    self.feedButton.layer.borderWidth = 0.5;
    self.feedButton.titleLabel.font = [UIFont systemFontOfSize:14];
    self.feedButton.layer.borderColor = [RGB(199, 200, 201) CGColor];
    [self.feedButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    self.feedButton.titleLabel.font = [UIFont systemFontOfSize:14];
    [self.feedButton setTitle:@"意见反馈" forState:UIControlStateNormal]; [self.feedButton addTarget:self action:@selector(feedBackAction) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.feedButton];
    UIImageView *feedbackImage = [[UIImageView alloc]initWithFrame:CGRectMake((ScreenWidth-50)/2-14, _backgroundImageView.frame.size.height-14, 10, 10)];
    feedbackImage.image = [UIImage imageNamed:@"商务咨询弹窗提醒-小三角"];
    [self.feedButton addSubview:feedbackImage];
    ...
    [styleChangeButton setImage:[UIImage imageNamed:@"icon_语音"] forState:UIControlStateNormal];
    [styleChangeButton setImage:[UIImage imageNamed:@"icon_键盘"] forState:UIControlStateSelected];
    ...
    if (feedBack == 5) {
        //转变输入样式
        UIButton *styleButton = [[UIButton alloc] init];
        styleButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
        [styleButton setImage:[UIImage imageNamed:@"从小容-切换内容按钮"] forState:UIControlStateNormal];
        [styleButton setImage:[UIImage imageNamed:@"从小容聊天详情-键盘"] forState:UIControlStateSelected];
        [styleButton addTarget:self action:@selector(keyAction:) forControlEvents:UIControlEventTouchUpInside];
        
        EaseChatToolbarItem *styleChangeItem = [[EaseChatToolbarItem alloc] initWithButton:styleButton withView:nil];
        
        [self setInputViewLeftItems:@[styleChangeItem,styleItem]];
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"icon_表情"] forState:UIControlStateNormal];
    //    [self.faceButton setImage:[UIImage imageNamed:@"EaseUIResource.bundle/icon_表情"] forState:UIControlStateHighlighted]; 点击高亮图片
    //    [self.faceButton setImage:[UIImage imageNamed:@"EaseUIResource.bundle/chatBar_keyboard"] forState:UIControlStateSelected];    选中状态图片
    ...
    [self.moreButton setImage:[UIImage imageNamed:@"icon_添加"] forState:UIControlStateNormal];
...
#pragma mark -商务咨询按钮-
-(void)keyAction:(UIButton *)button
{
    //录音状态下，不显示底部扩展页面
    ...
    [self bringSubviewToFront:self.feedButton];
    [self bringSubviewToFront:self.adviceButton];
    if (button.selected) {
        self.adviceButton.hidden = YES;
        self.feedButton.hidden = YES;
        button.selected = NO;
        [[NSNotificationCenter defaultCenter]postNotificationName:@"closeChatTool" object:@""];
    }else
    {
        button.selected = YES;
        self.adviceButton.hidden = NO;
...
#pragma mark -商务咨询按钮-
-(void)adviceAction:(UIButton *)button
{
    if (button.selected) {
        button.selected = NO;
    }else
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(adviceView:)]) {
        [_delegate adviceView:button.selected];
    }
    NSLog(@"点击了商务咨询");
}
#pragma mark -意见反馈按钮-
-(void)feedBackAction
{
    if (_delegate && [_delegate respondsToSelector:@selector(feedBackView)]) {
        [_delegate feedBackView];
    }
    NSLog(@"点击了意见反馈");
}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    if ([text isEqualToString:@"\n"]) {
        if ([self.delegate respondsToSelector:@selector(didSendText:)]) {
            NSLog(@"====================数组=====%@ ",_atUserModelArray);
            
//            for (AtOtherDetailModel *model in _atUserModelArray) {
//                NSString *FollowName;
//                if (model.name.length) {
//                    FollowName = [NSString stringWithFormat:@"@%@ ",model.name];
//                }else{
//                    FollowName = [NSString stringWithFormat:@"@%@ ",model.nickName];
//                }
//                NSLog(@"=========================%@ ",self.inputTextView.text);
//                
//                if ( [self.inputTextView.text rangeOfString:FollowName].location != NSNotFound){
//                    self.inputTextView.text  = [self.inputTextView.text stringByReplacingOccurrencesOfString:FollowName withString:[NSString stringWithFormat:@"@[%@]",model.id1]];
//                }
//            }
            
            NSLog(@"%@ 发送文字 === commentStr",self.inputTextView.text);
    ...
    if ([text isEqualToString:@"@"]) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 10)
        {
            if (range.length !=1) {
                ...
                if (_delegate && [self.delegate respondsToSelector:@selector(inputAtTextInRange:)]) {
                    [self.delegate inputAtTextInRange:location];
                }
            }
        }else
        {
...
-(void)setAtUserModelArray:(NSArray *)atUserModelArray
{
    _atUserModelArray = atUserModelArray;
}
