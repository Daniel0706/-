...
//#import "YYText.h"
//#import "ConversationShareFileViewController.h"
//#import "DownloadFileViewController.h"
//#import "MemberDataModel.h"
//#import "AppendixModel.h"
...
@interface EaseMessageViewController ()<EaseMessageCellDelegate,QLPreviewControllerDataSource,QLPreviewControllerDelegate>
{
...
@property (nonatomic,copy) NSMutableAttributedString *replaceArrStr;

@property (nonatomic,copy) NSMutableAttributedString *yyText;
...
- (void)viewDidLoad {
    ...
    [[EaseBaseMessageCell appearance] setSendBubbleBackgroundImage:[[UIImage imageNamed:@"对话气泡-我"] stretchableImageWithLeftCapWidth:5 topCapHeight:28]];
    [[EaseBaseMessageCell appearance] setRecvBubbleBackgroundImage:[[UIImage imageNamed:@"对话气泡-对方"] stretchableImageWithLeftCapWidth:12 topCapHeight:28]];
...
#pragma mark --- 发送附件
- (void)sendFileMessageWithMessageBody:(EMMessage *)message{
    [self _sendMessage:message];
    
    //    [[EMClient sharedClient].chatManager asyncSendMessage:message progress:^(int progress) {
    //    } completion:^(EMMessage *message, EMError *error) {
    //        NSLog(@"message == %@",error.errorDescription);
    //    }];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    // 清空聊天记录
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(deleteAllMessage) name:@"deleteAllMessage" object:nil];
...
//    [self setupEmotion];
     [self performSelector:@selector(setupEmotion) withObject:nil afterDelay:0.1];
}
...
- (void)_fileMessageCellSelected:(id<IMessageModel>)model
{
    ...
    EMDownloadStatus downloadStatus = [body downloadStatus];
    NSLog(@"downloadStatus ===%u === %@",downloadStatus,body.localPath);
    
    dispatch_block_t block = ^{
        _filePath = body.localPath;
        NSArray *fileArray = [model.fileName componentsSeparatedByString:@"."];
        NSString *fileExtension = fileArray.lastObject;
        if ([fileExtension isEqualToString:@"avi"]||[fileExtension isEqualToString:@"mp4"]||[fileExtension isEqualToString:@"mpe"]||[fileExtension isEqualToString:@"mpeg"]||[fileExtension isEqualToString:@"mpg"]||[fileExtension isEqualToString:@"mpg4"]||[fileExtension isEqualToString:@"rmvb"]||[fileExtension isEqualToString:@"sgp"]||[fileExtension isEqualToString:@"vod"]||[fileExtension isEqualToString:@"mkv"]||[fileExtension isEqualToString:@"wmv"]){
        ...
        }else if ([fileExtension isEqualToString:@"m3u"]||[fileExtension isEqualToString:@"m4a"]||[fileExtension isEqualToString:@"m4b"]||[fileExtension isEqualToString:@"m4p"]||[fileExtension isEqualToString:@"mp2"]||[fileExtension isEqualToString:@"mp3"]||[fileExtension isEqualToString:@"mpga"]||[fileExtension isEqualToString:@"ogg"]||[fileExtension isEqualToString:@"wav"]||[fileExtension isEqualToString:@"wma"]){
    ...
    if (downloadStatus == EMDownloadStatusDownloading) {
        [self showHint:NSEaseLocalizedString(@"message.downloadingFile", @"downloading file, click later")];
        return;
    }else if (downloadStatus == EMDownloadStatusPending){
        ...
        [[EMClient sharedClient].chatManager asyncDownloadMessageAttachments:model.message progress:nil completion:^(EMMessage *message, EMError *error) {
            ...
            if (!error) {
                ...
                [weakSelf showHint:@"下载失败"];
            }
        }];
        
        
        //        DownloadFileViewController *downloadVC = [[DownloadFileViewController alloc] init];
        //        downloadVC.model =  [[EaseMessageModel alloc] initWithMessage:model.message];
    ...
    else if (downloadStatus == EMDownloadStatusFailed)
    {
        ...
        return;
    }else if (downloadStatus == EMDownloadStatusSuccessed){
        block();
    }
}
...
//                    
//                    [strongSelf.tableView reloadData];//1
//                    
                     ...
                     append:(BOOL)isAppend
{
    ...
    dispatch_async(_messageQueue, ^{
        ...
        for (int i = 0; i < formattedMessages.count; i++) {
            id object = [formattedMessages objectAtIndex:i];
            if (![object isKindOfClass:[NSString class]]) {
                id<IMessageModel>model = object;
                
                model.text = [self heightWithModel:model];
            }
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if ([model.message.ext[@"is_create_group"] boolValue] && model.bodyType == EMMessageBodyTypeText) {
            NSString *TimeCellIdentifier = [EaseMessageTimeCell cellIdentifier];
            EaseMessageTimeCell *timeCell = (EaseMessageTimeCell *)[tableView dequeueReusableCellWithIdentifier:TimeCellIdentifier];
        ...
        if ([model.message.ext[@"is_create_online"] boolValue] && model.bodyType == EMMessageBodyTypeText) {
            NSString *TimeCellIdentifier = [EaseMessageTimeCell cellIdentifier];
            EaseMessageTimeCell *timeCell = (EaseMessageTimeCell *)[tableView dequeueReusableCellWithIdentifier:TimeCellIdentifier];
        ...
        if ([model.message.ext[@"em_revoke"] boolValue]) {
            if (model.isSender) {
                NSString *TimeCellIdentifier = [EaseMessageTimeCell cellIdentifier];
                EaseMessageTimeCell *timeCell = (EaseMessageTimeCell *)[tableView dequeueReusableCellWithIdentifier:TimeCellIdentifier];
                ...
                timeCell.title = [NSString stringWithFormat:@"%@撤回了一条消息",model.nickname];
                return timeCell;
        ...
        sendCell.modelArray = self.modelArray;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if ([model.message.ext[@"is_create_group"] boolValue])
        {
            //消息撤销提示
            return self.timeCellHeight;
        }
        if ([model.message.ext[@"is_create_online"] boolValue])
        {
            //消息撤销提示
            return self.timeCellHeight;
        }
...
//        NSString *thing = [self heightWithModel:model];
//        NSLog(@"7777777777777========%@",thing);
        return [EaseBaseMessageCell cellHeightWithModel:model];
    }
}
-(NSString *)heightWithModel:(id<IMessageModel>)model
{
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    [paragraphStyle setLineSpacing:1];//调整行间距
    NSDictionary *attDic = @{NSFontAttributeName:[UIFont systemFontOfSize:15],NSForegroundColorAttributeName:[UIColor blueColor]};
    NSDictionary *msgAttDic = @{NSFontAttributeName:[UIFont systemFontOfSize:15],NSForegroundColorAttributeName:RGB(102, 102, 102)};
    if (model.text) {
        //字符串过滤
        //        model.text = [NSString filterHTML:model.text];
        //        model.text=[model.text stringByReplacingOccurrencesOfString:@"&nbsp;"withString:@" "];
        _yyText = [[NSMutableAttributedString alloc] initWithString:model.text attributes:msgAttDic];
        //调整行间距
        [_yyText addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [_yyText length])];
        NSError *error = NULL;
        NSString *zhengzeStr = @"\\@\\[[^\\]]+\\]";
        NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:zhengzeStr options:kNilOptions error:&error];
        ...
        if (results.count != 0) {
            for (NSTextCheckingResult *checkResult in results) {
                ...
                NSString *userIDStr = [model.text substringWithRange:strRange];
//                for (MemberDataModel *contactModel in  self.modelArray) {
//                    if ([userIDStr isEqualToString:[NSString stringWithFormat:@"@[%@]",contactModel.memberID]]) {
//                        NSString *followName;
//                        followName = [NSString stringWithFormat:@"@%@ ",contactModel.memberName];
//                        _replaceArrStr = [[NSMutableAttributedString alloc] initWithString:followName attributes:attDic];
//                        //调整行间距
//                        [_replaceArrStr addAttribute:NSParagraphStyleAttributeName value:paragraphStyle range:NSMakeRange(0, [_replaceArrStr length])];
//                        
//                        NSRange idStrRange = [_yyText.string rangeOfString:userIDStr];
//                        [_yyText replaceCharactersInRange:idStrRange withAttributedString:_replaceArrStr];
//                    }
//                }
                
            }
        }
        model.text = _yyText.string;
    }else{
        model.text = @"";
    }
    NSLog(@"123456789087654324567=========%@",model.text);
    return model.text;
}
...
- (void)moreViewAudioCallAction:(EaseChatBarMoreView *)moreView
{
    // Hide the keyboard
    [self.chatToolbar endEditing:YES];
//    ConversationShareFileViewController *fileViewController = [[ConversationShareFileViewController alloc] init];
//    fileViewController.fileDelegate = self;
//    fileViewController.chatType = self.conversation.type;
//    fileViewController.conversationId = self.conversation.conversationId;
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            ...
            model.avatarImage = [UIImage imageNamed:@"登录-默认头像"];
...
//    }
    ...
    NSString *avator = [localData.userHeadImgUrl stringByReplacingOccurrencesOfString: IMAGE_HEADURL withString:@""];
    NSString *messageStr = [self changeMessageStrWithMessageStr:text];
    NSDictionary *ext = @{@"useravatar":avator,
                          @"usernick":localData.name,
                          @"em_apns_ext":@{@"em_push_title":messageStr}};
    [self sendTextMessage:text withExt:ext];
}
                         ...
                         andAddress:(NSString *)address
{
    ...
    NSDictionary *ext = @{@"useravatar":avator,
                          @"usernick":localData.name,
                          @"em_apns_ext":@{@"em_push_title":[NSString stringWithFormat:@"%@分享了一个地理位置",localData.name]}};
...
- (void)sendImageMessageWithData:(NSData *)imageData
{
    ...
    NSDictionary *ext = @{@"useravatar":avator,
                          @"usernick":localData.name,
                          @"em_apns_ext":@{@"em_push_title":[NSString stringWithFormat:@"%@分享了一张图片",localData.name]}};
                             ...
                             duration:(NSInteger)duration
{
    ...
    NSDictionary *ext = @{@"useravatar":avator,
                          @"usernick":localData.name,
                          @"em_apns_ext":@{@"em_push_title":@"您收到一条语音消息"}};
    id progress = nil;
    if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:progressDelegateForMessageBodyType:)]) {
...
- (void)sendVideoMessageWithURL:(NSURL *)url
{
    ...
    NSDictionary *ext = @{@"useravatar":avator,
                          @"usernick":localData.name,
                          @"em_apns_ext":@{@"em_push_title":@"您收到一条视频消息"}};
    id progress = nil;
    if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:progressDelegateForMessageBodyType:)]) {
...
- (NSString *) changeMessageStrWithMessageStr:(NSString *)messageStr{
    
    NSMutableAttributedString *yyText;
    NSMutableAttributedString *replaceArrStr;
    yyText = [[NSMutableAttributedString alloc] initWithString:messageStr];
    //调整行间距
    
    NSError *error = NULL;
    NSString *zhengzeStr = @"\\@\\[[^\\]]+\\]";
    NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:zhengzeStr options:kNilOptions error:&error];
    NSArray *results = [regex matchesInString:messageStr options:0 range:NSMakeRange(0, [messageStr length])];
    if (results.count != 0) {
        for (NSTextCheckingResult *checkResult in results) {
            NSRange strRange = [checkResult range];
            NSString *userIDStr = [messageStr substringWithRange:strRange];
//            for (MemberDataModel *contactModel in  self.modelArray) {
//                if ([userIDStr isEqualToString:[NSString stringWithFormat:@"@[%@]",contactModel.memberID]]) {
//                    NSString *followName;
//                    followName = [NSString stringWithFormat:@"@%@ ",contactModel.memberName];
//                    replaceArrStr = [[NSMutableAttributedString alloc] initWithString:followName];
//                    //调整行间距
//                    NSRange idStrRange = [yyText.string rangeOfString:userIDStr];
//                    [yyText replaceCharactersInRange:idStrRange withAttributedString:replaceArrStr];
//                }
//            }
        }
        messageStr = yyText.string;
        return messageStr;
    }else{
        return messageStr;
    }
